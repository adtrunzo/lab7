...
// modified October, 2021 (edges drawn in print_pretty())
// v. 1.02
//----------------------------------------------------------------------------
...
AVLNode<T> Q =  AVLNode<T>(NULL);
AVLNode<T> P = AVLNode<T>(NULL);
Q.setLeft(node_just_inserted->getLeft());
Q.setRight(node_just_inserted->getRight());
T key = node_just_inserted->getKey();
Q.setKey(key);
Q.setParent(node_just_inserted->getParent());
Q.setNumber(node_just_inserted->getNumber());
P.setLeft(Q.getParent()->getLeft());
P.setRight(Q.getParent()->getRight());
P.setParent(Q.getParent()->getParent());
P.setNumber(Q.getParent()->getNumber());
key = Q.getParent()->getKey();
P.setKey(key);

  if (P.getLeft() == node_just_inserted){
    P.balanceFactor--;
    root->balanceFactor--;
  }

  else P.balanceFactor++;
  while ((P.getKey() != root->getKey()) && (P.balanceFactor != 2 && P.balanceFactor != -2)){
    key = P.getKey();    
    node_just_inserted->setKey(key);
    node_just_inserted->setParent(P.getParent());
    node_just_inserted->setLeft(P.getLeft());
    node_just_inserted->setNumber(P.getNumber());
    node_just_inserted->setRight(P.getRight());
    //Q.setKey(key);
    //Q.setParent(P.getParent());
    //Q.setLeft(P.getLeft());
    //Q.setRight(P.getRight());
    //Q.setNumber(P.getNumber());
    P.setLeft(P.getParent()->getLeft());
    P.setRight(P.getParent()->getRight());
    key = P.getParent()->getKey();
    P.setKey(key);
    P.setNumber(P.getParent()->getNumber());
    P.setParent(P.getParent()->getParent());
    }
    if (node_just_inserted->balanceFactor == 0){
	return;
    }
    ...
    key = P.getKey();
    root->setKey(key);
    root->setParent(P.getParent());
    root->setLeft(P.getLeft());
    root->setNumber(P.getNumber());
    root->setRight(P.getRight());
// (2) Rebalance via rotations
  if (P.balanceFactor == 2){
    if (node_just_inserted==P.getLeft()){
	rotate_left(node_just_inserted);
	node_just_inserted->balanceFactor=0;
	rotate_right(root);
	P.balanceFactor = 0;
	...
	rotate_right(node_just_inserted);
	node_just_inserted->balanceFactor = 0;	
	rotate_left(root);
	P.balanceFactor = 0;
	root->balanceFactor = 0;
...
void AVLTree<T>::print_pretty()
{
  ...
  for (Q.push(root), last_level = 0; !Q.empty(); Q.pop()) {
    ...
    for (int i = last_column; i < current->print_column; i++) {
      if (left && i == current->print_column - offset)
	cout << "\/";
      else if (left && i >= current->print_column - offset)
	cout << "-";
      else
	cout << " ";
    }
